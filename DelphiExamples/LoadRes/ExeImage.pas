...
{ TExeImage }
  ...
  private
    ...
    function LoadResources(ResList: TResourceList): Pointer;  
  public
    constructor CreateImage(AOwner: TComponent; const AFileName: string);
    destructor Destroy; override;
    function GetFirstIcon: Pointer;
...
function TExeImage.LoadResources(ResList: TResourceList): Pointer;
var
  I: Integer;
Begin
  for I := 0 to ResList.Count - 1 do
    with ResList[I] do begin
      if IsList then LoadResources(List)
        ...
        if ResList[I].ResType = rtIconEntry then fIconData := ResList[I];
    end;
End;

function TExeImage.GetFirstIcon: Pointer;
Begin
  Result := nil;
  fIconData := nil;
  LoadResources(Self.GetResourceList);
  if fIconData <> nil then Result := fIconData;
End;
